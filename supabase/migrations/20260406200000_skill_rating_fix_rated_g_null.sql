-- rated_g: SELECT INTO assigns NULL when player_ratings has no row (no rows returned), so k_factor/delta became NULL.
-- (Previous: pg_safeupdate WHERE true on _champ_pairs.)
