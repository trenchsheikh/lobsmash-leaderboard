-- Coach assessments: require a non-empty trimmed note for every attribute (same six keys as scores).

drop function if exists public.submit_coach_assessment(uuid, jsonb, text, jsonb);

create or replace function public.submit_coach_assessment(
  p_request_id uuid,
  p_attribute_scores jsonb,
  p_venue text default null,
  p_attribute_notes jsonb default null
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  if p_attribute_notes is null then
    raise exception 'attribute notes required';
  end if;

  if (select count(*) from jsonb_object_keys(v_notes)) <> 6 then
    raise exception 'attribute notes must include all six attributes';
  end if;

  foreach k in array required_keys loop
    if not (v_notes ? k) then
      raise exception 'missing attribute note for %', k;
    end if;
    if jsonb_typeof(v_notes -> k) <> 'string' then
      raise exception 'invalid attribute note for %', k;
    end if;
    v_note := trim(v_notes ->> k);
    if length(v_note) < 1 then
      raise exception 'empty attribute note for %', k;
    end if;
    if length(v_note) > 600 then
      raise exception 'attribute note too long for %', k;
    end if;
  end loop;

  if exists (
    select 1
    from jsonb_object_keys(v_notes) as nk(key)
    where nk.key <> all (required_keys)
  ) then
  ...
  set
    ...
    coach_verified_attribute_notes = v_notes
...
comment on column public.coach_assessments.attribute_notes is
  'JSON map keyed like attribute_scores; each value is required coach rationale (non-empty string).';
