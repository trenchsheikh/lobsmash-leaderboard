-- League sessions: competitive (default) updates global skill; friendly does not.

alter table public.sessions
  add column if not exists match_kind text not null default 'competitive'
  check (match_kind in ('competitive', 'friendly'));

alter table public.sessions
  add column if not exists share_location text;

alter table public.sessions
  add column if not exists share_duration_minutes int
  check (share_duration_minutes is null or share_duration_minutes > 0);

alter table public.sessions
  add column if not exists share_restriction text;

alter table public.sessions
  add column if not exists scheduled_at timestamptz;

comment on column public.sessions.match_kind is 'competitive: global skill rating applies when completed; friendly: no player_ratings change';

-- Full body from 20260406200000_skill_rating_fix_rated_g_null.sql + friendly guard + match_kind in select.
