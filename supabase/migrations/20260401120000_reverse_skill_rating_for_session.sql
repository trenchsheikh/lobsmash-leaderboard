-- Reverse skill deltas for a completed session (same math as apply_skill_rating_for_session, negated).
-- Used before editing games/teams/court-1 data so apply_skill_rating_for_session can run again afterward.
-- Security: league owner/admin only (matches app server actions).
